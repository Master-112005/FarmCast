...
Deployment notes:
- In the full Docker stack, the broker reaches backend at `backend:5000` through `docker/mosquitto.compose.conf`.
- When running services manually outside Compose, keep `MQTT_BROKER_URL`, broker callback host, and backend port aligned.
...
- `docker-compose.yml`: full-stack local Docker Compose entrypoint for frontend, backend, ML, MQTT, and MySQL.
- `docker/`: MQTT broker ACL/config used by the full-stack Compose deployment.
- `mqtt/`: local broker runtime data/log mount directories generated by Docker.
...
- Configuration drift risk: Docker MQTT configuration is centralized in `docker/mosquitto.compose.conf`; keep firmware/backend topic contracts aligned with it.
...
Source-focused snapshot of all project files. Generated or local runtime folders such as `.git/`, `.venv-ml/`, `.vs/`, `node_modules/`, `dist/`, `.pio/`, `.pytest_cache/`, `__pycache__/`, `mqtt/data/`, `mqtt/log/`, `backend/uploads/`, `farmcast-ml/data/`, and `farmcast-ml/logs/` are omitted for readability.
...
|   |   |-- 017-add-pending-wifi-update-columns.js
|   |   |-- 018-add-firmware-version-to-devices.js
|   |   `-- 019-allow-null-soil-coordinates.js
|   |-- package.json
|   |-- package-lock.json
|   |-- README.md
|   |-- seeders/
